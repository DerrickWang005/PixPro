...
class ImageFolderLMDB(data.Dataset):
    def __init__(self, file_dir, transform, linear=False):
        self.db_path = file_dir
        self.linear = linear
        self.transform = transform
        self.env = None
        if 'train.lmdb' in self.db_path:
            self.length = 1281167
        elif 'val.lmdb' in self.db_path:
            self.length = 50000
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.linear:
            ...
            img_q, coord_q = self.transform[0](image)
            img_k, coord_k = self.transform[1](image)
            img = [img_q, img_k, coord_q, coord_k]
            return img
